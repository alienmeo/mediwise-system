...
# Kết nối PostgreSQL Neon thông qua DATABASE_URL trên Render
...
def direct_user_history():
    ...
    try:
        # Lấy toàn bộ lịch sử gần nhất trong database
        histories = AssessmentHistory.query.order_by(
            AssessmentHistory.created_at.desc()
        ).all()
        ...
        for h in histories:
            details_obj = {
                "recommendations": "Chưa có khuyến cáo chi tiết."
            }
                ...
                try:
                    ...
                    if isinstance(parsed_result, dict):
                        if 'recommendations' in parsed_result:
                            ...
                            'details' in parsed_result
                            and isinstance(parsed_result['details'], dict)
                        ):
                            if 'recommendations' in parsed_result['details']:
                                details_obj['recommendations'] = (
                                    parsed_result['details']['recommendations']
                                )
...
def direct_delete_feedback(feedback_id):
    ...
    try:
        ...
        requester_username = request.headers.get(
            'X-Username', ''
        ).strip().lower()
        ...
        if requester_username and feedback.username:
            if requester_username != feedback.username.strip().lower():
                return jsonify({
                    "error": "Bạn không có quyền xóa đánh giá của người khác!"
                }), 403
